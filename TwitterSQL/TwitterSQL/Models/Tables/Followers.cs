...
namespace TwitterSQL.Models.Tables
{
    public class Followers : ITable
    {
        ...
        private async Task<IList<CoreTweet.User>> GetRawResult()
        {
            ...
            var result = await tokens.Followers.ListAsync(screen_name: userName, count: count > 200 ? 200 : count);
            ...
            while (returnList.Count < count  && result.NextCursor != 0)
            {
                ...
                result = await tokens.Followers.ListAsync(screen_name: userName, count: requestCount, cursor: result.NextCursor);
                returnList.AddRange(result.ToList());
            }
