...
namespace TwitterSQL.Controls
{
    ...
    public class AutoCompleteView : ContentView
    {
        ...
        /// </summary>
        public static readonly BindableProperty SuggestionsHeightRequestProperty = BindableProperty.Create<AutoCompleteView, double>(p => p.SuggestionsHeightRequest, 125, BindingMode.TwoWay, null, SuggestionHeightRequestChanged);
        ...
        /// </summary>
        public static readonly BindableProperty TextIsEnableProperty = BindableProperty.Create<AutoCompleteView, bool>(p => p.TextIsEnable, true, BindingMode.TwoWay, null, TextIsEnabledChanged);
        ...
        private static void TextValueChanged(BindableObject obj, string oldPlaceHolderValue, string newPlaceHolderValue)
        {
            ...
            if (control != null && !string.IsNullOrEmpty(newPlaceHolderValue))
            {
                ...
                var cleanedNewPlaceHolderValue = newPlaceHolderValue.ToLowerInvariant();

                if (cleanedNewPlaceHolderValue.Contains(","))
                {
                    cleanedNewPlaceHolderValue = cleanedNewPlaceHolderValue.Split(',').Last().Trim();
                }
                else
                {
                    cleanedNewPlaceHolderValue = cleanedNewPlaceHolderValue.Split(' ').Last();
                }
                ...
                if (!string.IsNullOrEmpty(cleanedNewPlaceHolderValue) && control.Suggestions != null)
                {
                    ...
                    if (filteredSuggestions.Count > 0)
                    {
                        ...
                        //control._lstSuggestions.HeightRequest = control._lstSuggestions.RowHeight*
        ...
        private static void TextIsEnabledChanged(BindableObject obj, bool oldIsEnabledValue, bool newIsEnabledValue)
        {
            var autoCompleteView = obj as AutoCompleteView;
            if (autoCompleteView != null)
            {
                autoCompleteView._entText.IsEnabled = newIsEnabledValue;
            }
        }
    }
}